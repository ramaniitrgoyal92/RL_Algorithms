...
class DDP(object):
	...
	def forward_pass_sim(self, render=0, std=0, pr=False):
		...
		for t in range(0, self.N):
			...
			if t == 0:
				self.X_p[t] = run_vanderpol(yinit=self.X_p_0.flatten(), tfinal=1, forcing=ctrl.flatten(), mu=2)[-1].reshape(np.shape(self.X_p[0]))

			else:
				self.X_p[t] = run_vanderpol(yinit=self.X_p[t-1].flatten(), tfinal=1, forcing=ctrl.flatten(), mu=2)[-1].reshape(np.shape(self.X_p[0]))
